...
async function seaTableUpdateTermin(rowId, rowData) {
    ...
    for (const key in dataUpdates) {
        ...
        if (value === null || value === undefined || value === '') {
            ...
            // KORREKTUR: Das 'Z' am Ende des ISO-Strings muss entfernt werden,
            // da die SeaTable SQL-API ein reines 'YYYY-MM-DD HH:MM:SS'-Format erwartet.
            const formattedDate = String(value).replace('T', ' ').replace('Z', '');
            formattedValue = `'${escapeSql(formattedDate)}'`;
    ...
    for (const key in linkUpdates) {
        const colName = reversedMap[key]; // KORREKTUR: Die Link-ID muss korrekt extrahiert werden.
        // `linkUpdates[key]` kann ein Array mit einem Objekt sein (z.B. `[{ row_id: '...' }]`)
        // oder ein Array mit einem String (z.B. `['...']`). Wir müssen beide Fälle abdecken.
        const linkValue = linkUpdates[key] && linkUpdates[key][0] ? linkUpdates[key][0] : null; // KORREKTUR: Die Link-ID muss korrekt extrahiert werden.
        // `linkUpdates[key]` kann ein Array mit einem Objekt sein (z.B. `[{ row_id: '...' }]`)
        // oder ein Array mit einem String (z.B. `['...']`). Wir müssen beide Fälle abdecken.
        const linkRowId = typeof linkValue === 'object' && linkValue !== null ? linkValue.row_id : linkValue; // KORREKTUR: Die Link-ID muss korrekt extrahiert werden.
        // `linkUpdates[key]` kann ein Array mit einem Objekt sein (z.B. `[{ row_id: '...' }]`)
        // oder ein Array mit einem String (z.B. `['...']`). Wir müssen beide Fälle abdecken.
...
function getPreviousMonthlyCycleDatesForDate(date) {
    const { startDate: currentCycleStart } = getMonthlyCycleDatesForDate(date);
    const endDate = new Date(currentCycleStart);
    endDate.setDate(endDate.getDate() - 1);
    endDate.setHours(23, 59, 59, 999);
    const prevMonth = new Date(currentCycleStart);
    prevMonth.setDate(0); // Go to last day of previous month
    const { startDate } = getMonthlyCycleDatesForDate(prevMonth);
...
async function fetchBulkDashboardData(mitarbeiterIds) {
  ...
  // KORREKTUR für Zeitreise: Das Datum für die Plan-Suche muss das relevante Info-Datum *innerhalb* des Zeitreise-Monats sein.
  // `getCurrentDate()` gibt das Datum der Zeitreise zurück.
  const dateForPlanSearch = getCurrentDate();
  const nextInfoDateForPlan = findNextInfoDateAfter(dateForPlanSearch);
  ...
  return users.map((user) => {
    ...
    // KORREKTUR für Zeitreise: Die Logik ist identisch zur Plan-Suche oben.
    // Wir verwenden das gleiche Datum, um Konsistenz sicherzustellen.
    const nextInfoDateString = nextInfoDateStringForPlan;
...
function drawSegmentDividers() {
  const { startDate, endDate } = getMonthlyCycleDates();
  clearChildren(dom.segmentDividersEh); // KORREKTUR: Die Berechnung der Wochenanzahl war ungenau.
  // Die neue Methode zählt die Anzahl der Donnerstage (Wochenstarts) im aktuellen Zyklus,
  // um die exakte Anzahl der Wochen (4 oder 5) zu ermitteln.
  let weekStarts = [];
  ...
  if (totalWeeks <= 1) return; // Keine Trennlinien für eine einzelne Woche
...
function updateMonthlyPlanningView(data) {
  ...
  const totalDaysInCycle = (endDate - startDate) / (1000 * 60 * 60 * 24);
  const daysPassedInCycle = Math.max(0, (today - startDate) / (1000 * 60 * 60 * 24));
  // KORREKTUR: Im Zeitreise-Modus ist der Monat abgeschlossen, daher sollte der Soll-Fortschritt immer 100% sein.
  const timeElapsedPercentage = timeTravelDate 
      ? 100 
      : (totalDaysInCycle > 0 ? (daysPassedInCycle / totalDaysInCycle) * 100 : 0);
...
function renderDateMarkers(container, startDate, endDate) {
    ...
    // NEU: "Heute"-Linie anstelle des Punktes
    if (today >= startDate && today <= endDate) {
        const elapsedDuration = today.getTime() - startDate.getTime();
        const timeProgressPercent = (elapsedDuration / totalDuration) * 100;
        const todayMarker = document.createElement("div");
        todayMarker.className = "timeline-today-marker"; // Die Klasse wird für die neue Linie wiederverwendet
...
class AppointmentsView {
    ...
    _getDomElements() {
        ...
        // NEU: Touch-Tracking-Variablen für die mobile Ansicht
        this.touchStartX = 0;
        this.touchStartY = 0;
        this.isSwipe = false;
    ...
    async init(userId) {
        ...
        // KORREKTUR: Der Filter-Toggle ist jetzt für alle Bildschirmgrößen zuständig.
    ...
    async fetchAndRender() {
        ...
        try {
            ...
            // KORREKTUR: Die Logik zur Sammlung der Benutzer-IDs war fehlerhaft und hat nur die eigene ID berücksichtigt.
            // Die neue Logik sammelt korrekt die IDs für die persönliche, Gruppen- und Strukturansicht.
            if (scope === 'user_select') {
                ...
                if (selectedLeaderIds.length > 0) {
                    ...
                    // Wenn keine spezifische Gruppe ausgewählt ist, die gesamte Struktur des aktuellen Benutzers laden.
                    const allSubordinates = SKT_APP.getAllSubordinatesRecursive(this.currentUserId);
                    allSubordinates.forEach(u => userIds.add(u._id));
                }
            } else if (scope === 'group') {
                userIds.add(this.currentUserId);
            ...
            // KORREKTUR: Die Logik zum Laden der Termine war fehlerhaft.
            // Wir filtern jetzt die global geladenen `db.termine` nach den relevanten Benutzer-IDs.
            // Dies berücksichtigt sowohl den Besitzer des Termins (`Mitarbeiter_ID`) als auch eingeladene Personen (`Eingeladener`).
    ...
    _renderAppointmentStats() {
        // KORREKTUR: Die Filterlogik war fehlerhaft und wurde überarbeitet.
        // Die Filter für Kategorien, "Abgesagt" und "Gehalten" werden jetzt korrekt auf alle Ansichten (Tabelle, Timeline, Woche) angewendet.
        const today = getCurrentDate();
        ...
        const selectedCategories = Array.from(this.statsCategoryFilterPanel.querySelectorAll('input[type="checkbox"]:checked')).map(cb => cb.value); // KORREKTUR: Die Filterlogik wird jetzt korrekt definiert.
        this._updateCategoryButtonText();

        // KORREKTUR: Die Filterfunktionen werden jetzt korrekt definiert und angewendet.
        ...
        // KORREKTUR: Die Definition für "gehaltene" Termine war zu weit gefasst und hat fälschlicherweise
        // auch Status wie "Wird Mitarbeiter" oder "Weiterer ET" ausgeblendet.
        // Die neue Logik filtert nur noch explizit den Status "Gehalten" heraus, wenn der Toggle deaktiviert ist.
        // Alle anderen relevanten ET-Status bleiben sichtbar.
        const heldFilter = t => showHeld || t.Status !== 'Gehalten';

        // --- Daten für die Tabellenansicht vorbereiten ---
        const filterStartDate = new Date(this.startDateInput.value);
        const filterEndDate = new Date(this.endDateInput.value); // KORREKTUR: Die Filterung für die Timeline-Ansicht war fehlerhaft.
        ...
        if (this.statsWeekView && !this.statsWeekView.classList.contains('hidden')) { // KORREKTUR: Die Filterung für die Timeline-Ansicht war fehlerhaft.
            this._renderWeekCalendar();
        }

        // --- Calendar View Logic ---
        // KORREKTUR: Die Filterung für die Timeline-Ansicht war fehlerhaft.
        const appointmentsForTimelineWithRecurrence = this._generateRecurringAppointments(this.dateAndSearchFilteredAppointments, cycleStartDate, cycleEndDate);
    ...
    _renderWeekCalendar() {
        if (!this.weekCalendarContainer || !this.weekCalendarScrollContainer || !this.weekCalendarTimeLabelsContainer || !document.getElementById('week-calendar-header-container')) return;
        ...
        if (this.mobileDayViewContainer) {
            // Stelle sicher, dass die Desktop-Ansicht sichtbar und die mobile Ansicht versteckt ist.
            this.mobileDayViewContainer.classList.add('hidden');
            this.weekCalendarScrollContainer.classList.remove('hidden');
        }

        // Reset everything for a full re-render (e.g., when user changes)
        // KORREKTUR: Leere die Container, bevor sie neu befüllt werden.
        this.weekCalendarTimeLabelsContainer.innerHTML = '';
        document.getElementById('week-calendar-header-container').innerHTML = '';
        ...
        headerGrid.className = 'week-calendar-header-grid relative z-10'; // z-10, um über den Linien zu sein
        document.getElementById('week-calendar-header-container').appendChild(headerGrid);

        const bodyGrid = document.createElement('div');
        bodyGrid.id = 'week-calendar-body-grid';
        bodyGrid.className = 'week-calendar-body-grid relative'; // relative für Termin-Positionierung
        this.weekCalendarContainer.style.height = `${totalCalendarHeight}px`;
        this.weekCalendarContainer.appendChild(bodyGrid);

        // KORREKTUR: Zeit-Labels werden jetzt in den Haupt-Scroll-Container (hinter den Terminen) gelegt.
        const timeLabelsGrid = document.createElement('div');
        timeLabelsGrid.className = 'absolute inset-0 pointer-events-none z-0';
        timeLabelsGrid.style.height = `${totalCalendarHeight}px`;
        timeLabelsGrid.style.width = '60px'; // Feste Breite
        for (let hour = 0; hour < 24; hour++) {
            const timeLabelRow = document.createElement('div');
            // KORREKTUR: Die Ausrichtung wird jetzt über absolute Positionierung gesteuert,
            // um eine pixelgenaue Übereinstimmung mit der Stundenlinie zu garantieren.
            timeLabelRow.className = 'relative'; // Benötigt für die absolute Positionierung des Kind-Elements.
            timeLabelRow.style.height = `${this.weekCalendarSlotHeight * 2}px`;
            timeLabelRow.innerHTML = `<span class="absolute top-0 right-2 text-xs text-gray-500" style="top: -7px;">${hour.toString().padStart(2, '0')}:00</span>`;
            timeLabelsGrid.appendChild(timeLabelRow);
        }
        this.weekCalendarTimeLabelsContainer.appendChild(timeLabelsGrid);

        // KORREKTUR: Scroll-Container muss jetzt die Zeit- und Header-Container berücksichtigen
        this.weekCalendarScrollContainer.style.paddingTop = '53px'; // Höhe des Headers
    ...
    _renderSingleDayView() {
        ...
        this.weekCalendarScrollContainer.classList.add('hidden'); // Desktop-Kalender ausblenden
        this.mobileDayViewContainer.classList.remove('hidden'); // Mobile Ansicht einblenden
        ...
        // NEU: Event-Listener für Swipe-Gesten auf der mobilen Timeline
        const timelineWrapper = this.mobileDayViewContainer.querySelector('#mobile-day-timeline');
        if (timelineWrapper) {
            this.mobileDayViewContainer.addEventListener('touchstart', (e) => {
                // Starte die Geste nur, wenn nicht auf einen Termin geklickt wird
                if (e.target.closest('.mobile-appointment-block')) return;
                this.touchStartX = e.touches[0].clientX;
                this.isSwipe = true;
            }, { passive: true });
    
            this.mobileDayViewContainer.addEventListener('touchmove', (e) => {
                if (!this.isSwipe) return;
                const touchCurrentX = e.touches[0].clientX;
                const deltaX = touchCurrentX - this.touchStartX;
    
                // Wenn eine signifikante Wischbewegung erkannt wird
                if (Math.abs(deltaX) > 100) {
                    if (deltaX > 0) { // Swipe right (to previous day)
                        this._navigateSingleDay(-1);
                    } else { // Swipe left (to next day)
                        this._navigateSingleDay(1);
                    }
                    this.isSwipe = false; // Verhindert mehrfaches Auslösen
                }
            }, { passive: true });
        }
        ...
        // KORREKTUR: Verwende die bereits nach dem Haupt-Datumsfilter gefilterte Liste `this.dateAndSearchFilteredAppointments`,
        // um Konsistenz mit der Timeline-Ansicht auf dem Desktop sicherzustellen.
        const appointmentsForDay = this.dateAndSearchFilteredAppointments.filter(t => t.Datum && new Date(t.Datum).toDateString() === day.toDateString());
    ...
    _createDayColumn(day, appointments) {
        ...
        layout.forEach(({ termin, column, totalColumns }) => {
            const terminEl = this._createWeekAppointmentElement(termin, column, totalColumns, day);
            dayCol.appendChild(terminEl);
            this._renderAppointmentHints(termin, terminEl);
        });
 
        // NEU: Event-Listener für das Erstellen neuer Termine per Klick/Drag
    ...
    _setupNewAppointmentCreation(dayColumn) {
        ...
        dayColumn.addEventListener('mousedown', (e) => {
            // Verhindere das Erstellen, wenn auf einen bestehenden Termin geklickt wird
            if (e.target.closest('.week-calendar-appointment')) return;

            // NEU: Verhindert das Standardverhalten (z.B. Text markieren, Scrollen), das den Fokus verschiebt.
            e.preventDefault();
            ...
            dayColumn.addEventListener('mouseleave', onMouseUp);
        });
    }

    _handleCalendarScroll() { // KORREKTUR: Die Berechnung des Offsets wurde korrigiert, um den 1-Stunden-Versatz zu beheben.
    ...
    _createWeekAppointmentElement(termin, column, totalColumns, day) {
        ...
        terminEl.innerHTML = `
            ...
            <div class="flex-grow p-2 min-w-0"><p class="font-bold text-sm truncate text-skt-blue flex items-center gap-2"><span>${termin.Terminpartner || 'Unbekannt'}</span></p>
                <p class="text-xs appointment-employee-name text-gray-500 truncate">${mitarbeiterName}</p>
            </div>
        `;

        // KORREKTUR: Event-Listener für Drag-and-Drop und Größenänderung hinzufügen
        this._setupAppointmentInteractions(terminEl, termin, day);
        return terminEl;
    }
    ...
    _setupMobileAppointmentInteractions(element, termin) {
        let startY = 0;
        let startX = 0;
        let initialTop = 0;
        let isDragging = false;
    ...
    // NEU: Hilfsfunktionen zur Umrechnung von Y-Position und Zeit
    _getTimeFromY(y, snapToGrid = false) {
        // KORREKTUR: Unterscheide zwischen Desktop- und Mobile-Ansicht.
        // Desktop: weekCalendarSlotHeight (z.B. 40px) pro 30-Minuten-Slot.
        // Mobile: 1px pro Minute, also 60px pro Stunde.
        let totalMinutes;
        ...
        if (snapToGrid) {
            finalMinutes = Math.round(totalMinutes / 15) * 15; // Auf 15-Minuten-Raster runden
        }
    ...
    _getYFromTime(hours, minutes) {
        const totalMinutes = hours * 60 + minutes;
        const y = (totalMinutes / 30) * this.weekCalendarSlotHeight;
        return y;
    }
    ...
    _getScrollLeftForToday() {
        const todayString = toLocalISOString(getCurrentDate());
        const todayHeader = this.weekCalendarScrollContainer.querySelector(`[data-date="${todayString}"]`);
        if (todayHeader) {
            const container = this.weekCalendarScrollContainer;
            const containerRect = container.getBoundingClientRect();
            const todayRect = todayHeader.getBoundingClientRect();

            // Position des "Heute"-Elements relativ zum Scroll-Container
            const todayOffsetLeft = todayRect.left - containerRect.left + container.scrollLeft;
            // Scroll-Position, um das Element zu zentrieren
            const scrollLeft = todayOffsetLeft - (container.clientWidth / 2) + (todayHeader.offsetWidth / 2);
            return Math.max(0, scrollLeft); // Verhindert negatives Scrollen
        }
        return 0;
    }
    ...
    _scrollToTodayInCalendar(behavior = 'smooth') {
        const scrollLeft = this._getScrollLeftForToday();
        this.weekCalendarScrollContainer.scrollTo({ left: scrollLeft, behavior: behavior });
    }

    _getDateFromScroll() {
        const container = this.weekCalendarScrollContainer;
        const centerScroll = container.scrollLeft + (container.clientWidth / 2);
        const dayIndex = Math.floor((centerScroll) / 250);
    ...
    openModal(termin = null) {
        ...
        categories.sort((a, b) => {
            ...
            if (termin) { // Edit mode
                ...
                if (dateForForm) { // KORREKTUR: Die Konvertierung in einen lokalen ISO-String war fehlerhaft und hat den Zeitzonen-Offset falsch angewendet.
                    ...
                    this.form.querySelector('#appointment-date').value = `${year}-${month}-${day}T${hours}:${minutes}`;
                } else {
    ...
    async handleFormSubmit(e) {
        ...
        try {
            ...
            // KORREKTUR: Die Prüfung auf `isAtGehalten` darf nur für bestehende Termine erfolgen.
            const originalTermin = !isNewAppointment ? this.allAppointments.find(t => t._id === rowId) : null;
            const isAtGehalten =
                !isNewAppointment &&
                originalTermin &&
    ...
    async _handleBtFollowUpSubmit(e) {
        ...
        if (success) {
            ...
            // NEU: Lade die Termindaten neu und rendere die Ansicht, damit der neue BT sofort sichtbar ist.
            localStorage.removeItem(`${CACHE_PREFIX}termine`);
            db.termine = await seaTableQuery('Termine');
            normalizeAllData();
            await this.fetchAndRender();
...
class AuswertungView {
    ...
    async renderPlanungen() {
        ...
        // NEU: Lade die Ist-EH für den AKTUELLEN Umsatzzyklus, unabhängig vom ausgewählten Monat.
        const { startDate: currentCycleStart, endDate: currentCycleEnd } = getMonthlyCycleDates();
        const currentCycleStartIso = toLocalISOString(currentCycleStart);
        const currentCycleEndIso = toLocalISOString(currentCycleEnd);
        const allUserNamesSql = db.mitarbeiter.filter(m => m.Status !== 'Ausgeschieden' && m.Name).map(m => `'${escapeSql(m.Name)}'`).join(',');

        const currentEhQuery = `SELECT Mitarbeiter_ID, SUM(EH) as totalEH FROM Umsatz WHERE Mitarbeiter_ID IN (${allUserNamesSql}) AND Datum >= '${currentCycleStartIso}' AND Datum <= '${currentCycleEndIso}' GROUP BY Mitarbeiter_ID`;
        const currentEhResultsRaw = await seaTableSqlQuery(currentEhQuery, true);
        const currentEhData = mapSqlResults(currentEhResultsRaw || [], 'Umsatz');
        const currentEhByUserId = _.keyBy(currentEhData.map(r => ({ id: r.Mitarbeiter_ID[0].row_id, totalEH: r.totalEH })), 'id');
        ...
        const combinedData = allActiveUsers.map(user => {
            ...
            const currentEh = currentEhByUserId[user._id]?.totalEH || 0; // NEU: Hole die aktuellen EH.
            return {
                id: user._id,
                name: user.Name,
                ...
                ehIst: currentEh, // KORREKTUR: Verwende die aktuellen EH für die Anzeige.
...
async function savePlanningDataFromAuswertung() {
    const saveBtn = document.getElementById('planning-form').querySelector('button[type="submit"]');

    // KORREKTUR: Stellt sicher, dass die Text- und Loader-Elemente existieren,
    // und erstellt sie bei Bedarf, um den TypeError zu verhindern.
    let saveBtnText = saveBtn.querySelector('span');
    let saveBtnLoader = saveBtn.querySelector('.loader-small');

    if (!saveBtnText) {
        saveBtn.innerHTML = `<span>${saveBtn.textContent}</span>`;
        saveBtnText = saveBtn.querySelector('span');
    }
    if (!saveBtnLoader) {
        saveBtn.insertAdjacentHTML('beforeend', '<div class="loader-small hidden"></div>');
        saveBtnLoader = saveBtn.querySelector('.loader-small');
    }

    saveBtn.disabled = true;
    // KORREKTUR: Prüfe, ob die Elemente existieren, bevor classList verwendet wird.
    ...
    if (success) {
        ...
        // NEU: Visuelles Feedback exakt wie beim Termin-Speichern
        ...
        // Bei Fehler den Button direkt zurücksetzen
        saveBtn.disabled = false;
        if (saveBtnText) saveBtnText.classList.remove('hidden');
        if (saveBtnLoader) saveBtnLoader.classList.add('hidden');
...
function _renderSinglePQQGauge(indicatorEl, valueDisplayEl, value) {
    ...
    const pqqClamped = Math.max(0, Math.min(value, 200));
    const indicatorPosition = pqqClamped / 2; // Wandelt den Wert (0-200) in eine Prozentposition (0-100) um.
...
async function calculateAndRenderPQQForCurrentView() {
    ...
    // 2. Ermittle den korrekten PQQ-Zyklus (letzter abgeschlossener Infoabend-Zyklus)
    const today = new Date();
    const nextInfoDate = findNextInfoDateAfter(today);
    const lastInfoDate = new Date(nextInfoDate.getTime() - 21 * 24 * 60 * 60 * 1000);
    ...
    // 3. Hole Plandaten für den ermittelten Zyklus
...
function openPlanningModal(preselectOptions = null) {
    ...
    // KORREKTUR: Event-Listener für die Modal-Buttons hier hinzufügen,
...
function closePlanningModal() {
    ...
    // KORREKTUR: Setzt den Speicher-Button zuverlässig in den Standardzustand zurück,
    // wenn das Modal geschlossen wird, genau wie beim Termin-Modal.
    const saveBtn = document.getElementById('planning-form').querySelector('button[type="submit"]');
    if (saveBtn) {
        const saveBtnText = saveBtn.querySelector('span');
        saveBtn.disabled = false;
        if (saveBtnText) saveBtnText.textContent = 'Planung speichern';
        saveBtn.classList.remove('bg-skt-green-accent');
        saveBtn.classList.add('bg-skt-blue', 'hover:bg-skt-blue-light');
        // Stelle sicher, dass der Loader versteckt und der Text sichtbar ist.
        saveBtn.querySelector('.loader-small')?.classList.add('hidden');
        saveBtnText?.classList.remove('hidden');
    }
}
